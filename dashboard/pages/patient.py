"""
Patient explorer — Personal Health Coach (Powered by Machine Learning).
...
# IMPORTAR O NOSSO MOTOR DE ML
from pages.ml_utils import run_predictions
...
# Mantemos as fórmulas originais como mecanismos de Fallback (Segurança)
...
STAGE_DESC = {
    "No Diabetes": "AI Assessment: Low risk profile based on your current metrics.",
    "Pre-Diabetes": "Your values are slightly elevated. Lifestyle changes can prevent progression.",
    "Diabetes": "AI Assessment: High risk patterns identified. Medical consultation strongly advised."
}
...
def zone_hba1c(v):
    if v < 5.7: return "good"
    if v < 6.5: return "mid"
    return "bad"

def zone_glucose(v):
    if 70 <= v <= 100: return "good"
    if (60 <= v < 70) or (100 < v < 126): return "mid"
    return "bad"

def zone_bmi(v):
    if 18.5 <= v <= 24.9: return "good"
    if (17.0 <= v < 18.5) or (25.0 <= v < 30.0): return "mid"
    return "bad"
...
def chart_gauge(value, color):
    fig = go.Figure(go.Indicator(
        ...
        title={"text": "AI Risk Score Prediction", "font": {"size": 12, "color": "rgba(255,255,255,0.6)"}},
...
def chart_activity_diet_bar(activity, diet, df):
    ...
    for i, (cat, uv, pv, rv) in enumerate(zip(categories, user_vals, pop_vals, ref_vals)):
        ...
        fig.add_trace(go.Scatter(
            x=[pv, pv], y=[i - 0.32, i + 0.32], mode="lines",
            line=dict(color="rgba(255,255,255,0.55)", width=2, dash="dot"),
            hovertemplate=f"<b>{cat}</b><br>Population Average: {pv:.1f}<extra></extra>", showlegend=False,
        ))
        fig.add_trace(go.Scatter(
            x=[rv, rv], y=[i - 0.32, i + 0.32], mode="lines",
            line=dict(color="rgba(255,255,255,0.9)", width=2),
            hovertemplate=f"<b>{cat}</b><br>Recommended Value: {rv:.1f}<extra></extra>", showlegend=False,
        ))
    ...
    fig.update_layout(
        ...
        yaxis=dict(tickmode="array", tickvals=y_positions, ticktext=categories, range=[-0.6, len(categories) - 0.4], showgrid=False, zeroline=False),
...
def chart_activity_risk(df, user_activity, user_risk):
    ...
    user_bin = pd.cut(pd.Series([user_activity]), bins=bins, labels=labels, include_lowest=True).iloc[0]
    ...
    fig.add_hline(
        y=user_risk, line_dash="dot", line_color=C_WARM, line_width=2,
        annotation_text=f"Your Predicted Risk: {user_risk:.1f}", annotation_font_size=10, annotation_font_color=C_WARM,
    )
    fig.update_layout(title="Diabetes Risk vs Physical Activity", xaxis_title="Physical Activity (min/week)", yaxis_title="Median Diabetes Risk", height=300, margin=dict(l=50, r=10, t=50, b=50), **PLOTLY_LAYOUT)
    axis_style(fig)
    st.plotly_chart(fig, width="stretch", config={"displayModeBar": False})
...
def chart_risk_vs_age_group(df, user_age, user_risk):
    ...
    fig.add_hline(y=user_risk, line_dash="dot", line_color=C_WARM, line_width=2, annotation_text=f"Your Predicted Risk: {user_risk:.0f}", annotation_font_size=10, annotation_font_color=C_WARM, annotation_position="bottom right")
...
def chart_bmi_risk(df, user_bmi, user_risk):
    ...
    user_group = pd.cut(pd.Series([user_bmi]), bins=bins, labels=labels, include_lowest=True).iloc[0]
    color_map = {label: (C_ORANGE if label == user_group else C_BLUE) for label in labels}

    fig = px.box(
        tmp.dropna(subset=["bmi_group", "diabetes_risk_score"]), x="bmi_group", y="diabetes_risk_score",
        category_orders={"bmi_group": labels}, color="bmi_group", color_discrete_map=color_map,
        points="outliers", labels={"bmi_group": "BMI Group", "diabetes_risk_score": "Diabetes Risk"}, title="Diabetes Risk vs BMI",
    )
    ...
    if pd.notna(group_median):
        fig.add_scatter(x=[user_group], y=[group_median], mode="markers", marker=dict(color=C_WARM, size=11, line=dict(color="white", width=1)), name="Your BMI Group Median", hovertemplate=f"Your BMI Group: {user_group}<br>Median Risk: {group_median:.1f}<extra></extra>", showlegend=False)

    fig.add_hline(y=user_risk, line_dash="dot", line_color=C_WARM, line_width=2, annotation_text=f"Your Predicted Risk: {user_risk:.1f}", annotation_font_size=10, annotation_font_color=C_WARM)
    fig.update_layout(title="Diabetes Risk vs BMI", xaxis_title="BMI Group", yaxis_title="Diabetes Risk", height=360, margin=dict(l=30, r=20, t=50, b=30), **PLOTLY_LAYOUT)
    axis_style(fig)
    st.plotly_chart(fig, width="stretch", config={"displayModeBar": False})

def chart_sleep_risk(sleep, risk, df):
    agg = df.groupby("sleep_hours_per_day")["diabetes_risk_score"].mean().reset_index()
    fig = px.line(agg, x="sleep_hours_per_day", y="diabetes_risk_score", labels={"sleep_hours_per_day": "Sleep Hours", "diabetes_risk_score": "Average Diabetes Risk"}, title="Impact of Sleep on Average Risk")
    fig.update_traces(line_color=C_BLUE, line_width=3)
    fig.add_scatter(x=[sleep], y=[risk], mode="markers", marker=dict(color=C_WARM, size=12, line=dict(width=2, color="white")), name="You", hovertemplate=f"Sleep: {sleep}h<br>Your Predicted Risk: {risk}<extra></extra>")
...
def show(back_fn):
    ...
    with st.sidebar:
        ...
        hba1c       = st.slider("Average Blood Sugar (HbA1c %)", 4.0, 14.0, 5.5, step=0.1, help="Glycated haemoglobin. Normal range is below 5.7%.")
        glucose     = st.slider("Fasting Blood Sugar (mg/dL)", 60, 350, 90, help="Blood sugar levels measured after fasting for at least 8 hours.")
        postprandial = st.slider("Post-Meal Sugar (mg/dL)", 60, 400, 140, help="Blood sugar levels measured 2 hours after a meal.")
        sbp         = st.slider("Top Blood Pressure (Systolic mmHg)", 80, 210, 120, help="The top number on your blood pressure reading.")
        chol        = st.slider("Total Cholesterol (mg/dL)", 100, 380, 180, help="Your total cholesterol level measured via a blood test.")
        ...
        diet     = st.slider("Diet Quality Score (0-10)", 0.0, 10.0, 6.0, step=0.5, help="Rate how healthy your diet is from 0 to 10.")
        ...
        st.markdown("---")
        submit = st.button("Generate AI Report", width="stretch")
    ...
    with h_title:
        st.markdown('<div class="pat-title">🩺 TeaBetes — AI Health Explorer</div>', unsafe_allow_html=True)
        st.markdown('<div class="pat-sub">Adjust the controls in the sidebar — our Machine Learning models will predict your risk in real-time.</div>', unsafe_allow_html=True)
    with h_back:
        if st.button("← Home", key="pat_back_top"): back_fn()

    if not st.session_state.get("patient_submitted", False):
        st.info("Please fill out your health details in the sidebar menu on the left, then click 'Generate AI Report' to consult our predictive models.")
    ...
    # 1. Preparação de Dados para o ML
    bmi   = weight / ((height / 100) ** 2)
    grp   = age_group(age)
    ws_label, ws_color = weight_status(bmi)
    
    # Dicionário de input para o nosso motor preditivo
    user_inputs = {
        "age": age,
        "bmi": bmi,
        "hba1c": hba1c,
        ...
        "screen_time_hours_per_day": screen_h,
        "family_history_diabetes": 1 if family else 0,
        "diagnosed_hypertension": 1 if hypert else 0,
        "smoking_status": smoking,
        "age_groups": grp,
        "weight_status": ws_label
    }

    # 2. Execução da Inteligência Artificial
    diagnosis_pred, risk_score_pred = run_predictions(user_inputs)

    # Lógica Híbrida: Usa o ML se carregado com sucesso; senão, usa fórmula manual
    if risk_score_pred is not None:
        risk = round(min(max(float(risk_score_pred), 0), 100), 1)
    else:
        risk = estimate_risk(age, bmi, hba1c, glucose, sbp, family, hypert, smoking, activity, diet)
        
    if diagnosis_pred is not None:
        if diagnosis_pred == 1:
            stage, stage_color, stage_level = "Diabetes", C_RED, 3
        else:
            stage, stage_color, stage_level = "No Diabetes", C_GREEN, 1
    else:
    ...
    with col_status:
        st.markdown(f"""<div class="status-card" style="background:{stage_color}cc">
            <div class="status-lbl">AI Diagnostic Prediction</div><div class="status-val">{stage}</div><div class="status-desc">{STAGE_DESC.get(stage, "")}</div></div>""", unsafe_allow_html=True)
    ...
    st.markdown("<br><br>", unsafe_allow_html=True)
    st.markdown('<div class="sec-title">Your AI Risk & Lifestyle Context</div>', unsafe_allow_html=True)
    ...
    st.markdown("<br><small style='opacity:.28'>Indicative tool powered by Machine Learning — does not replace professional medical advice.</small>", unsafe_allow_html=True)